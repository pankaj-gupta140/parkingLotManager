...
public class MongoAdmin implements adminServices {
    ...
    public void setPassword(String password) {
        MongoAdmin.password = password;
    }

    @Override
    public String carsFromColor(String color) {
        try{
            FindIterable<Document> iteratorDoc = MongoDB.collection.find(Filters.eq("color",color));
            ...
            while (it.hasNext()) {
                Document d = it.next();
                System.out.println("\t\t "+ i++ +"-> " + d.getString("regNum"));
    ...
    public String slotOfCar(String regNumber) {
        try{
            ...
            FindIterable<Document> iteratorDoc = MongoDB.collection.find(Filters.eq("regNum",regNumber));
            // Getting the iterator
            Iterator<Document> it = iteratorDoc.iterator();
            if(!it.hasNext())
                return "NO RESULT FOUND";
            ...
            while(it.hasNext()) {
                Document d = it.next();
                System.out.println("\t\t "+ d.getInteger("floor")+" | "+d.getInteger("row")+"\t "+
                        d.getString("entryDate")+" | "+d.getString("entryTime")+"\t "+
                        d.getString("exitDate")+" | "+d.getString("exitTime"));
            }
    ...
    public void service(){
        try{
            ...
            boolean flag = true;
            while(flag){
                System.out.println("\t\t\t****** Mongo Admin Service Portal ******");
